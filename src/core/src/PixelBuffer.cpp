...
PixelBufferF::PixelBufferF(int width, int height, Rgbaf fill)
    : width_(width < 0 ? 0 : width), height_(height < 0 ? 0 : height) {
    const std::size_t count = static_cast<std::size_t>(width_) * static_cast<std::size_t>(height_);
    pixels_.assign(count, fill);
}

Rgbaf PixelBufferF::at(int x, int y) const noexcept {
    if (x < 0 || y < 0 || x >= width_ || y >= height_) return Rgbaf{};
    return pixels_[static_cast<std::size_t>(y) * static_cast<std::size_t>(width_) +
                   static_cast<std::size_t>(x)];
}

void PixelBufferF::set(int x, int y, Rgbaf c) noexcept {
    if (x < 0 || y < 0 || x >= width_ || y >= height_) return;
    pixels_[static_cast<std::size_t>(y) * static_cast<std::size_t>(width_) +
            static_cast<std::size_t>(x)] = c;
}

void PixelBufferF::fill(Rgbaf c) noexcept {
    for (auto& p : pixels_) p = c;
}

PixelBuffer16::PixelBuffer16(int width, int height, Rgba16 fill)
    : width_(width < 0 ? 0 : width), height_(height < 0 ? 0 : height) {
    const std::size_t count = static_cast<std::size_t>(width_) * static_cast<std::size_t>(height_);
    pixels_.assign(count, fill);
}

Rgba16 PixelBuffer16::at(int x, int y) const noexcept {
    if (x < 0 || y < 0 || x >= width_ || y >= height_) return Rgba16{};
    return pixels_[static_cast<std::size_t>(y) * static_cast<std::size_t>(width_) +
                   static_cast<std::size_t>(x)];
}

void PixelBuffer16::set(int x, int y, Rgba16 c) noexcept {
    if (x < 0 || y < 0 || x >= width_ || y >= height_) return;
    pixels_[static_cast<std::size_t>(y) * static_cast<std::size_t>(width_) +
            static_cast<std::size_t>(x)] = c;
}

void PixelBuffer16::fill(Rgba16 c) noexcept {
    for (auto& p : pixels_) p = c;
}
